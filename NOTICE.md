...
The offline fitting pipeline uses SeqIKPy 1.1.0, commit `f7f1dc9b09ce89c4f54b2005722d62f887623a7b`, Apache-2.0; its locomotion notebook supplies initial IK seeds. License: `public/licenses/SeqIKPy.txt`. Cite Özdil et al., _SeqIKPy: a Python package for inverse kinematics in insects_, JOSS 11(117), 8557 (2026), doi:10.21105/joss.08557. Our additional XML-offset refinement, comparisons, UI and experimental tracking controller are separate project work. No upstream learned behavioural controller is included.

## Original terrarium props

The half-open banana is original procedural geometry authored in `scripts/build-banana.py` using Blender. `assets/banana.blend` retains its editable meshes; `public/props/banana` contains the visual GLB and 94 short convex contact pieces derived from those same surfaces. These assets and the script are MIT licensed with this project. Their scale and botanical form are illustrative, not a scan or anatomical measurement.

The v0.4 free body additionally reuses the pinned Apache-2.0 NeuroMechFly anatomy and processed stepping template listed above. Passive tarsal hinges and adhesion follow FlyGym model precedents; our muscles and ideal recruitment remain explicit hypotheses.
