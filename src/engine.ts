...
export class FlyEngine {
  ...
  excite(index: number, value: number) {
    if (
    ...
    this.data.ctrl[index] = Math.max(
      this.manifest.muscles[index].ctrlrange[0],
      Math.min(1, value),
    );
  }
  release() {
  ...
  pulse(value = 0.35, duration = 0.05) {
    this.excite(this.selected, value);
    this.pulseEnd = this.data.time + duration;
    this.running = true;
  }
  step(count = 1) {
    for (let i = 0; i < count; i++) {
      ...
      if (this.data.time - this.previousSample >= 0.001 - 1e-9) this.record();
    }
  }
  advance(wallSeconds: number) {
    if (!this.running) return;
    this.remaining += Math.min(wallSeconds, 0.05) * this.speed;
    const steps = Math.min(
      150,
      Math.floor(this.remaining / this.model.opt.timestep),
    );
    this.step(steps);
    this.remaining -= steps * this.model.opt.timestep;
    this.remaining = Math.min(this.remaining, 0.02);
  }
  sample(): Sample {
    const i = this.selected;
    return {
      time: this.data.time,
      excitation: this.data.ctrl[i],
  ...
  record() {
    ...
    this.previousSample = this.data.time;
  }
  select(index: number) {
    this.selected = index;
    this.history = [];
