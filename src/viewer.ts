...
export class FlyViewer {
  ...
  constructor(
    ...
    const platform = new THREE.Mesh(
      new THREE.CylinderGeometry(4.6, 4.6, 0.08, 96),
      new THREE.MeshStandardMaterial({ color: 0xd7ddcd, roughness: 0.9 }),
    );
    ...
    for (let i = 0; i < m.ngeom; i++) {
      ...
      const id = m.geom_dataid[i],
        ...
        fa = m.mesh_faceadr[id] * 3,
        fn = m.mesh_facenum[id] * 3;
      const geometry = new THREE.BufferGeometry();
      geometry.setAttribute(
        "position",
        new THREE.Float32BufferAttribute(
          new Float32Array(m.mesh_vert.slice(va, va + vn)),
          3,
        ),
      );
      geometry.setIndex(
        new THREE.Uint32BufferAttribute(
          new Uint32Array(m.mesh_face.slice(fa, fa + fn)),
          1,
        ),
      );
      geometry.computeVertexNormals();
      const meshName = mj.mj_id2name(m, mj.mjtObj.mjOBJ_GEOM.value, i) || "";
      const wing = /wing/i.test(meshName),
        eye = /eye/i.test(meshName),
        leg = /coxa|femur|tibia|tarsus|trochanter/i.test(meshName);
      const color = eye
        ? 0x7e3026
        : wing
          ? 0xb6c7b5
    ...
    for (const muscle of engine.manifest.muscles) {
      ...
      if (sites.some((id) => id < 0))
        throw new Error("Anatomical tendon site missing: " + muscle.id);
      const geometry = new THREE.BufferGeometry();
      geometry.setAttribute(
        "position",
        new THREE.BufferAttribute(new Float32Array(sites.length * 3), 3),
      );
  ...
  focus(view: "body" | "leg") {
    ...
    if (view === "body") {
      ...
      this.controls.target.set(0.05, 0.65, 3.2);
      this.camera.position.set(-1.2, 3.4, 4.15);
    }
    this.controls.update();
  }
  toggleXray() {
    this.xray = !this.xray;
    for (const { mesh, wing } of this.geoms) {
      const mat = mesh.material as THREE.MeshStandardMaterial;
      mat.transparent = this.xray || wing;
      mat.opacity = this.xray ? 0.18 : wing ? 0.2 : 1;
      mat.depthWrite = !this.xray && !wing;
    }
    return this.xray;
  }
  update() {
    ...
    this.paths.forEach(({ line, sites }, i) => {
      ...
      mat.color.set(i === this.engine.selected ? 0xda582b : 0x6d8063);
      mat.opacity = i === this.engine.selected ? 1 : this.xray ? 0.32 : 0.12;
    });
    this.controls.update();
