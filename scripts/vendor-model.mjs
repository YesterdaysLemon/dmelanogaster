...
const xml = await readFile(resolve(source,path),'utf8');
...
const manifest=[];
for(const asset of assets){const data=await readFile(resolve(source,asset.source));await writeFile(resolve(root,asset.target),data);manifest.push({...asset,bytes:data.length,sha256:createHash('sha256').update(data).digest('hex')});}
await copyFile(resolve(source,'LICENSE'),resolve(root,'LICENSE-FlyMimic.txt'));
